...
from validation import couple_illness_to_cognition, couple_pain_to_cognition
...
def build_temporal_chain(context_name: str, num_timepoints: int = 5) -> list[Timepoint]:
    ...
    for i in range(1, num_timepoints):
        ...
        description = event_descriptions.get(
            context_name, [f"Continuing developments {i} periods after {context['event']}"]
        )[min(i - 1, 4)]
...
def validate_temporal_reference(
    entity_id: str, knowledge_item: str, timepoint_id: str, store
) -> dict[str, Any]:
    """
...
def validate_causal_chain_integrity(timepoints: list[Timepoint]) -> dict[str, Any]:
    ...
    for tp in timepoints:
        ...
        if tp.causal_parent and tp.causal_parent not in timepoint_dict:
            issues.append(
                f"Timepoint {tp.timepoint_id} references non-existent parent {tp.causal_parent}"
            )
        ...
        if tp.causal_parent:
            ...
            if parent and tp.timestamp <= parent.timestamp:
                issues.append(
                    f"Timepoint {tp.timepoint_id} ({tp.timestamp}) is not after parent {tp.causal_parent} ({parent.timestamp})"
                )
