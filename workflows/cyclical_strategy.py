...
class CyclicalStrategy:
    ...
    def __init__(self, config: TemporalConfig, llm_client, store):
        ...
        self.cycle_semantics: CycleSemantics | None = None
    ...
    def run(self) -> list[CyclicalPath]:
        ...
        print(f"\n{'=' * 80}")
        print("CYCLICAL MODE: Time Loop Simulation")
    ...
    def _interpret_cycle_semantics(self) -> CycleSemantics:
        ...
        description = getattr(self.config, "portal_description", None) or getattr(
            self.config, "scenario_description", "A cyclical narrative"
        )
        ...
        system_prompt = (
            "You are an expert at temporal narrative patterns and cyclical storytelling."
        )
...
7. variation_seeds: What changes between cycles (3-5 seeds)"""
    ...
    def _generate_cycle_state(
        ...
        position: int,
        escalation: float,
        archetype_state: CyclicalState | None = None,
    ) -> CyclicalState:
...
- escalation_assessment: Brief assessment of how stakes have changed"""
        ...
        for path_idx in range(self.path_count):
            ...
            for cycle_idx in range(1, self.loop_count):
                ...
                for pos in range(self.cycle_length):
                    ...
                    archetype = (
                        self.archetype_states[pos] if pos < len(self.archetype_states) else None
                    )
                    ...
                    for prophecy_record in self.prophecies:
                        if (
                            prophecy_record["path_idx"] == path_idx
                            and not prophecy_record["fulfilled"]
                            and prophecy_record["target_cycle"] <= cycle_idx
                        ):
                ...
                if cycle_idx % 2 == 0:
                    print(
                        f"    Cycle {cycle_idx + 1}/{self.loop_count}: escalation {escalation:.2f}"
                    )
    ...
    def _generate_prophecy(self, state: CyclicalState, cycle_idx: int) -> ProphecySchema | None:
        ...
        mechanism = (
            self.cycle_semantics.prophecy_mechanism
            ...
            if self.llm and composite > 0.2:
                try:
                    ...
                    path.coherence_score = (
                        composite * 0.6
                        + ((llm_result.cycle_coherence + llm_result.pattern_similarity) / 2) * 0.4
                    )
    ...
    def _check_pattern_consistency(self, path: CyclicalPath) -> float:
        ...
        for i in range(1, len(cycle_keys)):
            ...
            if prev_elements and curr_elements:
                overlap = len(prev_elements & curr_elements) / max(
                    1, len(prev_elements | curr_elements)
                )
...
4. issues: List of any problems"""
    ...
    def _generate_placeholder_cycle_state(
        ...
        return CyclicalState(
            ...
            entities=current_state.entities.copy() if current_state.entities else [],
            world_state={
                "key_events": [f"Cycle {cycle_index + 1} event"],
                "recurring_elements": [],
            },
