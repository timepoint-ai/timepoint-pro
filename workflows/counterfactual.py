...
def create_counterfactual_branch(
    ...
    if not intervention_timepoint:
        raise ValueError(
            f"Intervention point {intervention_point} not found in timeline {parent_timeline_id}"
        )
...
def apply_intervention_to_timepoint(
    ...
    if intervention.type == "entity_removal":
        ...
        if (
            hasattr(modified_tp, "entities_present")
            and intervention.target in modified_tp.entities_present
        ):
            modified_tp.entities_present.remove(intervention.target)
            # Modify event description to reflect the removal
            modified_tp.event_description = (
                f"{modified_tp.event_description} (Note: {intervention.target} was not present)"
            )
        ...
        if modifications:
            ...
            modified_tp.event_description = (
                f"{modified_tp.event_description} (Modified: {intervention.target} {mod_str})"
            )
        ...
        modified_tp.event_description = (
            f"{modified_tp.event_description} (Knowledge altered for {intervention.target})"
        )
...
def propagate_causality_from_branch(
    branch_timeline_id: str, intervention_timepoint: "Timepoint", store: "GraphStore"
) -> None:
    ...
    subsequent_timepoints = [
        tp for tp in parent_timepoints if tp.timestamp > intervention_timepoint.timestamp
    ]
...
def compare_timelines(
    baseline_timeline_id: str, counterfactual_timeline_id: str, store: "GraphStore"
) -> "BranchComparison":
    ...
    for i in range(min_length):
        if (
            baseline_timepoints[i].event_description
            != counterfactual_timepoints[i].event_description
        ):
    ...
    for i in range(min_length):
        ...
        if baseline_tp.event_description != counterfactual_tp.event_description:
            key_events_differed.append(
                f"{baseline_tp.timepoint_id}: '{baseline_tp.event_description}' -> '{counterfactual_tp.event_description}'"
            )

        # Check entity presence differences
        if hasattr(baseline_tp, "entities_present") and hasattr(
            counterfactual_tp, "entities_present"
        ):
            ...
            if baseline_entities != counterfactual_entities:
                ...
                if added or removed:
                    entity_states_differed.append(
                        f"{baseline_tp.timepoint_id}: added={list(added)}, removed={list(removed)}"
                    )
...
def find_first_divergence(baseline_timepoints: list, counterfactual_timepoints: list) -> str | None:
