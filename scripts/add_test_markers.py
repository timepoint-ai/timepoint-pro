...
def analyze_test_file(file_path: Path) -> set[str]:
    ...
    try:
        ...
        if (
            len(analyzer.imports) < 5
            and analyzer.fixture_count <= 1
            and "llm" not in str(analyzer.imports).lower()
        ):
            markers.add("unit")
        ...
        if (
            "workflows" in analyzer.imports
            or "ai_entity_service" in analyzer.imports
            or "storage" in analyzer.imports
        ):
            markers.add("system")
        ...
        if (
            re.search(r"time\.sleep", content)
            or re.search(r"for .* in range\([2-9]\d+", content)
            or "llm" in markers
        ):
            markers.add("slow")
