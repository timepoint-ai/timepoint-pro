...
class ClarificationEngine:
    ...
    def __init__(self):
        ...
        self.historical_keywords = {
            "constitutional", "convention", "apollo", "revere",
            "washington", "franklin", "battle", "war", "treaty"
        }

        self.business_keywords = {
            "board", "meeting", "ceo", "startup", "acquisition",
            "merger", "earnings", "shareholders", "strategy"
        }

        self.crisis_keywords = {
            "crisis", "emergency", "disaster", "accident", "failure",
            "explosion", "pressure", "urgent"
        }
        ...
        self.focus_keywords = {
            ...
            "knowledge_propagation": ["spread", "alert", "inform", "communicate", "share information"]
        }
    ...
    def detect_ambiguities(
        ...
        if not self._has_entity_count(description_lower):
            clarifications.append(Clarification(
                ...
                suggestions=["3-5 for detailed interactions", "10-20 for group dynamics", "20+ for large-scale scenarios"],
                priority=1,
        ...
        if not self._has_timepoint_count(description_lower):
            clarifications.append(Clarification(
                ...
                suggestions=["3-5 for quick scenarios", "10-15 for detailed evolution", "20+ for long narratives"],
                priority=1,
        ...
        if self._is_historical(description_lower) and not self._has_time_reference(description_lower):
            clarifications.append(Clarification(
                field="start_time",
                question="What specific date/time should this historical scenario start?",
                suggestions=["Provide ISO datetime (e.g., '1787-05-25T10:00:00')", "Provide year and time of day"],
                priority=2,
        ...
        if not self._has_temporal_mode_hint(description_lower):
            ...
            if any(keyword in description_lower for keyword in ["alternate", "what if", "counterfactual", "branching"]):
                clarifications.append(Clarification(
                    field="temporal_mode",
        ...
        if wants_variations and not has_variation_count:
            clarifications.append(Clarification(
                ...
                suggestions=["10-50 for quick exploration", "100-500 for training data", "500-1000 for comprehensive dataset"],
                priority=1,
    ...
    def _has_output_preference(self, description: str) -> bool:
        ...
        output_keywords = [
            "want to see", "show me", "output", "generate",
            "dialog", "conversation", "decision", "relationship"
        ]
    ...
    def _needs_animism(self, description: str) -> bool:
        ...
        animistic_entities = [
            "horse", "ship", "boat", "car", "train", "plane",
            "organization", "company", "corporation", "building",
            "city", "nation", "country"
        ]
    ...
    def _wants_variations(self, description: str) -> bool:
        ...
        variation_keywords = [
            "variation", "variations", "horizontal", "generate many",
            "different versions", "multiple scenarios"
        ]
    ...
    def get_clarification_summary(self, clarifications: List[Clarification]) -> str:
        ...
        if important:
            summary.append(f"Important ({len(important)}): " + ", ".join(c.field for c in important))

        if nice_to_have:
            summary.append(f"Optional ({len(nice_to_have)}): " + ", ".join(c.field for c in nice_to_have))
