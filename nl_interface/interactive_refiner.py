...
class InteractiveRefiner:
    ...
    def answer_clarifications(self, answers: dict[str, str]) -> dict[str, Any]:
        """
        ...
        for clarification in self.pending_clarifications:
            if clarification.field in answers:
                ...
                self.current_description = self.clarification_engine.answer_clarification(
                    clarification, answer, self.current_description
                )
...
"""
    ...
    def adjust_config(
        self, adjustments: dict[str, Any], regenerate: bool = False
    ) -> dict[str, Any]:
        """
    ...
    def reject_and_restart(self, reason: str | None = None) -> dict[str, Any]:
        """
    ...
    def run(self, description: str) -> dict[str, Any]:
        ...
        if result["clarifications_needed"]:
            ...
            for clarification in result["clarifications"]:
                ...
                print(
                    f"   Priority: {'Critical' if clarification.priority == 1 else 'Important' if clarification.priority == 2 else 'Optional'}"
                )
                print("   Suggestions:")
