...
def test_circadian_validation():
    ...
    # Create test entity
    entity = Entity(entity_id="test_entity", entity_type="person")
    ...
    result = validate_circadian_activity(
        entity, {"activity": "work", "timepoint": timepoint_afternoon, "circadian_config": config}
    )
...
def test_circadian_context_creation():
    ...
    print(
        f"  Morning context (9 AM): fatigue={morning_context.fatigue_level:.2f}, penalty={morning_context.energy_penalty:.2f}"
    )
