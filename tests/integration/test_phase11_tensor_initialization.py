...
def test_baseline_tensor_creation():
    ...
    # Create test entity
    entity = Entity(entity_id="sherlock_holmes", entity_type="human")
    entity.entity_metadata = {
        ...
        "cognitive_traits": {"prospection_ability": 0.95, "theory_of_mind": 0.9},
    }
...
def test_optional_prospection_triggering():
    ...
    detective_entity = Entity(
        ...
        entity_metadata={
            ...
            "cognitive_traits": {"prospection_ability": 0.5},
        },
    )
    ...
    config_with_prospection = {"metadata": {"prospection_config": {"modeling_entity": "moriarty"}}}

    should_trigger_config = should_trigger_prospection(
...
def test_prospection_params():
    ...
    timepoint = Timepoint(timepoint_id="tp1", timestamp=datetime.now(), event_description="Test")

    config = {"metadata": {"prospection_config": {"time_horizons": ["6h", "12h", "24h"]}}}
...
def test_integration_with_storage():
    ...
    entity = Entity(entity_id="test", entity_type="human", entity_metadata={})
    ...
    print(
        f"  ✓ Dimensions preserved: {len(context_decoded)}, {len(biology_decoded)}, {len(behavior_decoded)}"
    )
