...
class CharacterRoleplayFormatter(EntityEvolutionFormatter):
    ...
    def format_simulation(
        self, simulation_result: dict[str, Any], config: Any
    ) -> list[dict[str, Any]]:
        """
    ...
    def _find_character_entity(self, entities: list[Any]) -> Any | None:
    ...
    def _create_character_example(
        ...
        return {"prompt": prompt, "completion": completion, "context": context}
    ...
    def _build_character_prompt(
        ...
        event_description = getattr(
            timepoint, "event_description", "No event description available"
        )
        ...
        for exp in entity_exposures:
            ...
            if self._is_before_or_at(exp_tp, timepoint_id, previous_timepoints):
                ...
                exposure_history.append(
                    {"information": info, "source": source, "timepoint": exp_tp}
                )
...
6. Any new knowledge gained"""
    ...
    def _build_context_metadata(
        ...
        mechanisms_used = self._identify_mechanisms_used(entity, t0, t1, config, exposure_events)
        ...
        context = {
            ...
            "includes_dialog": hasattr(config, "outputs")
            and getattr(config.outputs, "include_dialogs", False),
            ...
            "scenario_description": getattr(config, "scenario_description", "")[:200]
            + "...",  # Truncate for size
        }
    ...
    def _generate_reasoning(self, entity: Any, timepoint: Any, new_knowledge: list[str]) -> str:
        ...
        return (
            f"Based on {len(new_knowledge)} new observation(s): {', '.join(new_knowledge[:3])}..."
        )
    ...
    def _extract_circadian_context(self, timepoint: Any) -> dict[str, Any]:
        ...
        if timestamp and hasattr(timestamp, "hour"):
            hour = timestamp.hour
            return {"hour": hour, "time_of_day": self._classify_time_of_day(hour)}
    ...
    def _identify_mechanisms_used(
        self, entity: Any, t0: Any, t1: Any, config: Any, exposure_events: dict[str, list[Any]]
    ) -> list[str]:
        """
        ...
        if hasattr(config, "temporal") and getattr(
            config.temporal, "enable_counterfactuals", False
        ):
