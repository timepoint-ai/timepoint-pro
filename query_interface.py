...
class QueryIntent(BaseModel):
    ...
    intervention_type: str | None = (
        None  # "entity_removal", "entity_modification", "event_cancellation"
    )
...
class QueryInterface:
    ...
    def clear_expired_cache(self) -> int:
        ...
        now = datetime.now()
        expired_keys = [
            key for key, (_, timestamp) in _query_cache.items() if now - timestamp >= CACHE_TTL
        ]
    ...
    def get_cache_stats(self) -> dict[str, int]:
        ...
        now = datetime.now()
        valid_entries = sum(
            1 for _, timestamp in _query_cache.values() if now - timestamp < CACHE_TTL
        )
    ...
    def parse_query(self, query: str) -> QueryIntent:
        ...
        timepoint_descriptions = [
            f"{tp.timepoint_id}: {tp.event_description[:50]}..." for tp in timepoints[:5]
        ]
...
Return only valid JSON, no other text."""
    ...
    def _parse_query_simple(self, query: str) -> QueryIntent:
        ...
        # Dialog/conversation queries
            ...
            word in query_lower
            for word in ["talk", "say", "said", "speak", "conversation", "told", "spoke"]
        ):
            info_type = "dialog"
            confidence += 0.2
        ...
        # Action/behavior queries
            ...
            word in query_lower
            for word in ["do", "did", "action", "activity", "perform", "take", "took"]
        ):
            info_type = "actions"
            confidence += 0.2
        ...
        # Relationship queries
            ...
            word in query_lower
            for word in ["interact", "relationship", "work with", "together", "alliance"]
        ):
        ...
        # Scene/atmosphere queries
            ...
            word in query_lower
            for word in ["atmosphere", "mood", "feeling", "vibe", "environment", "crowd", "scene"]
        ):
        ...
        if any(keyword in query_lower for keyword in counterfactual_keywords):
            ...
            if any(
                word in query_lower
                for word in ["absent", "missing", "not present", "removed", "was absent"]
            ):
    ...
    def synthesize_response(self, query_intent: QueryIntent, query_text: str = "") -> str:
        ...
        if not query_intent.target_entity:
            ...
            if query_intent.information_type in ["atmosphere", "environment", "crowd"] or any(
                word in getattr(query_intent, "_original_query", "").lower()
                for word in ["atmosphere", "mood", "scene", "environment", "crowd"]
            ):
            ...
            # Check if this is a multi-entity relationship query
                ...
                return (
                    "I couldn't identify which entity you're asking about. Available entities: "
                    + ", ".join(self._get_all_entity_names())
                )
        ...
        if self._resolution_level_value(entity.resolution_level) < self._resolution_level_value(
            required_resolution
        ):
        ...
        temporally_filtered_knowledge = self._filter_knowledge_by_time(
            knowledge_state, query_intent, entity.entity_id
        )
        relevant_knowledge = self._filter_relevant_knowledge(
            temporally_filtered_knowledge, query_intent
        )
        ...
        if relevant_knowledge:
            ...
            source_map = {
                event.information: (event.source, event.timestamp) for event in exposure_events
            }
            ...
            response_parts.append(
                f"{entity.entity_id} doesn't have specific knowledge about that topic in the current temporal simulation."
            )
    ...
    def _tensor_to_knowledge(
        self, tensor: np.ndarray, tensor_type: str, entity: Entity
    ) -> list[str]:
    ...
    def _interpret_context_tensor(self, tensor: np.ndarray, entity: Entity) -> list[str]:
            ...
            for _i, (category, value) in enumerate(zip(categories, tensor[:10], strict=False)):
                if abs(value) > 0.3:  # Significant knowledge in this area
                    ...
                    knowledge_items.append(
                        f"Has {intensity} knowledge of {category.replace('_', ' ')}"
                    )
        ...
        if "president" in role and len(tensor) > 5:
            if tensor[0] > 0.5:  # Historical events dimension
                knowledge_items.append(
                    "Well-versed in historical precedents and governmental affairs"
                )
    ...
    def _interpret_behavior_tensor(self, tensor: np.ndarray, entity: Entity) -> list[str]:
        ...
        if len(tensor) >= 5:
            ...
            trait_descriptions = {
                "openness": [
                    "closed-minded and traditional",
                    "somewhat open to new ideas",
                    "highly open and creative",
                ],
                "conscientiousness": [
                ...
                "extraversion": [
                    ...
                    "somewhat social",
                    "highly outgoing and energetic",
                ],
                "agreeableness": [
                    "competitive and assertive",
                    "moderately cooperative",
                    "highly compassionate and trusting",
                ],
                "neuroticism": [
                    "emotionally stable",
                    "moderately sensitive",
                    "highly anxious and emotional",
                ],
            }
        ...
        if "president" in role or "leader" in role:
            behavior_traits.append(
                "Leadership role suggests strong decision-making and social skills"
            )
        elif "general" in role:
            behavior_traits.append(
                "Military command experience indicates discipline and strategic thinking"
            )

        # Add temporal consistency note
        behavior_traits.append(
            "Behavioral patterns show temporal inertia (personality stability over time)"
        )

        return behavior_traits[:6]  # Limit to most relevant traits

    @track_mechanism("M5", "lazy_resolution_elevation")
    def _elevate_entity_resolution(
        ...
        try:
            ...
            population = self.llm_client.populate_entity(
                {
                    "entity_id": entity.entity_id,
                    "timestamp": latest_timepoint.timestamp.isoformat(),
                },
                enhanced_context,
            ...
            if target_resolution.value == "scene":
                print(
                    f"  📝 Scene LLM Response: {population.knowledge_state[0][:200]}..."
                    if population.knowledge_state
                    else "  📝 Scene LLM Response: (no knowledge generated)"
                )
                ...
                if population.knowledge_state:
                    print(
                        f"  💬 Dialog LLM Response: {population.knowledge_state[0][:200]}..."
                        if population.knowledge_state
                        else "  💬 Dialog LLM Response: (no knowledge generated)"
                    )
    ...
    def _filter_relevant_knowledge(
        self, knowledge_state: list[str], query_intent: QueryIntent
    ) -> list[str]:
    ...
    def _filter_knowledge_by_time(
        self, knowledge_state: list[str], query_intent: QueryIntent, entity_id: str
    ) -> list[str]:
    ...
    def _synthesize_timepoint_response(self, query_intent: QueryIntent) -> str:
        ...
        response_parts.append(
            f"[Timepoint: {query_intent.target_timepoint}, Confidence: {query_intent.confidence:.1f}]"
        )
    ...
    def _synthesize_relationship_response(self, query_intent: QueryIntent) -> str:
        ...
        response_parts = [
            f"**Relationship Analysis: {', '.join(e.replace('_', ' ').title() for e in entities)}**"
        ]
        ...
        if contradictions:
            ...
            for contradiction in contradictions[:3]:
                ...
                response_parts.append(
                    f"• {entity_a} and {entity_b} disagree on '{topic}' (severity: {severity:.1f})"
                )
        ...
        if dialogs:
            ...
            for dialog in dialogs[:2]:  # Limit to most recent
                participants = (
                    json.loads(dialog.participants)
                    if isinstance(dialog.participants, str)
                    else dialog.participants
                )
                participant_names = [p.replace("_", " ").title() for p in participants]
                response_parts.append(
                    f"• Conversation between {', '.join(participant_names)} at {dialog.timepoint_id}"
                )
    ...
    def _synthesize_scene_response(self, query_intent: QueryIntent) -> str:
        ...
        if query_intent.target_timepoint:
            ...
            for tp in timepoints:
                if any(
                    loc.lower() in query_lower
                    for loc in ["federal hall", "hall", tp.event_description.lower()]
                ):
                    timepoint = tp
                    break
        ...
        if (
            "atmosphere" in query_intent.information_type
            or "atmosphere" in getattr(query_intent, "_original_query", "").lower()
        ):
    ...
    def _get_scene_entities(self, timepoint_id: str) -> dict[str, Any | None]:
        """Retrieve scene entities for a timepoint"""
        # In a real implementation, this would query the database
        # For now, return None to trigger on-demand creation
        return {"environment": None, "atmosphere": None, "crowd": None}
    ...
    def _create_scene_entities_on_demand(self, timepoint: Timepoint) -> dict[str, any]:
        ...
        return {"environment": environment, "atmosphere": atmosphere, "crowd": crowd}
    ...
    def _describe_atmosphere(self, scene_entities: dict, timepoint: Timepoint) -> str:
        ...
        response_parts = [
            f"Atmosphere at {environment.location} during {timepoint.event_description.lower()}:"
        ]
        response_parts.append("")
    ...
    def _describe_environment(self, scene_entities: dict, timepoint: Timepoint) -> str:
        ...
        response_parts.append(
            f"• Lighting: {'bright' if environment.lighting_level > 0.7 else 'moderate' if environment.lighting_level > 0.4 else 'dim'}"
        )
        ...
        if environment.architectural_style:
            response_parts.append(
                f"• Architecture: {environment.architectural_style.replace('_', ' ')}"
            )
    ...
    def _describe_crowd(self, scene_entities: dict, timepoint: Timepoint) -> str:
        ...
        response_parts.append(
            f"• Noise level: {'loud' if crowd.noise_level > 0.7 else 'moderate' if crowd.noise_level > 0.4 else 'quiet'}"
        )
        ...
        try:
            ...
            if demo_comp and "gender_balance" in demo_comp:
                ...
                response_parts.append(
                    f"• Gender balance: {male_pct:.1%} male, {female_pct:.1%} female"
                )
    ...
    def _describe_scene_overview(self, scene_entities: dict, timepoint: Timepoint) -> str:
        ...
        if scene_entities["environment"]:
            ...
            response_parts.append(
                f"📍 Location: {env.location} ({env.architectural_style.replace('_', ' ')})"
            )
            response_parts.append(
                f"🌡️ Conditions: {env.ambient_temperature:.1f}°C, {'bright' if env.lighting_level > 0.7 else 'moderate'} lighting"
            )
        ...
        if scene_entities["atmosphere"]:
            ...
            response_parts.append(
                f"{valence_icon} Atmosphere: {'positive' if atm.emotional_valence > 0.2 else 'negative' if atm.emotional_valence < -0.2 else 'neutral'} mood, {energy_icon} {'high' if atm.energy_level > 0.7 else 'moderate' if atm.energy_level > 0.4 else 'low'} energy"
            )
        ...
        if scene_entities["crowd"]:
            crowd = scene_entities["crowd"]
            response_parts.append(
                f"👥 Crowd: {crowd.size} people, {crowd.movement_pattern} movement"
            )
    ...
    def detect_entity_gap(self, query: str, existing_entities: set[str]) -> str | None:
    ...
    def generate_entity_on_demand(self, entity_id: str, timepoint: Timepoint | None) -> Entity:
        ...
        timestamp_str = (
            timepoint.timestamp.strftime("%Y-%m-%d %H:%M") if timepoint else "Unknown date"
        )
...
Return only valid JSON."""
        ...
        try:
            ...
            entity = Entity(
                ...
                entity_metadata={
                    ...
                    "job_title": entity_data.get(
                        "role", "Unknown"
                    ),  # Store LLM's job title separately
            ...
            entity.physical_tensor = PhysicalTensor(
                age=entity_data.get("age", 35), pain_level=0.0, fever=36.5
            )
            ...
            knowledge_desc = (
                f"Present at {timepoint.event_description}" if timepoint else "Historical figure"
            )
            entity = Entity(
                entity_id=entity_id,
                entity_type="person",
                resolution_level=ResolutionLevel.TENSOR_ONLY,
                entity_metadata={"role": context["role"], "knowledge_state": [knowledge_desc]},
            )
            if timepoint:
                entity.temporal_span_start = timepoint.timestamp
                entity.temporal_span_end = timepoint.timestamp
    ...
    def _analyze_relationship_trajectories(
        self, entity_ids: list[str], timeline: list[dict]
    ) -> list[dict]:
    ...
    def _synthesize_counterfactual_response(
        self, query_intent: QueryIntent, query_text: str
    ) -> str:
        """Generate response for counterfactual/what-if queries using branching"""
        from schemas import Intervention
        from workflows import compare_timelines, create_counterfactual_branch

        try:
            # Get the baseline timeline
            baseline_timeline = self.store.get_timeline(
                "main_timeline"
            )  # Assuming main timeline exists
            ...
            intervention = None
            if (
                query_intent.intervention_type == "entity_removal"
                and query_intent.intervention_target
            ):
                intervention = Intervention(
                    type="entity_removal", target=query_intent.intervention_target, parameters={}
                )
                ...
                intervention = Intervention(
                    type="event_cancellation", target=intervention_point, parameters={}
                )
            else:
            ...
            if "entity_knowledge_delta" in comparison:
                ...
                if knowledge_changes:
                    response_parts.append(
                        f"• Knowledge changes: {len(knowledge_changes)} items affected"
                    )
                else:
            ...
            if "relationship_changes" in comparison:
                ...
                if rel_changes:
                    response_parts.append(
                        f"• Relationships: {len(rel_changes)} relationships affected"
                    )
                else:
            ...
            response_parts.append("")
            response_parts.append(
                "[Counterfactual analysis completed. This is a simulated branching scenario.]"
            )
