...
def get_archetype_by_id(archetype_id: str) -> ArchetypeSeed | None:
    """Get archetype by ID."""
...
def generate_archetype_tensor(
    archetype: ArchetypeSeed, llm_client: Any, include_training: bool = False, verbose: bool = True
    ...
    try:
        refined_tensor, maturity = populate_tensor_llm_guided(entity, timepoint, graph, llm_client)
    ...
    if include_training:
        try:
            ...
            if verbose:
                print("    -> Training skipped (requires store)")
        except Exception as e:
            if verbose:
...
def generate_all_archetype_tensors(
    llm_client: Any,
    archetypes: list[ArchetypeSeed] | None = None,
...
def get_best_archetype_match(
    role: str, entity_type: str, description: str = ""
) -> ArchetypeSeed | None:
    """
